...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--nproc', type=int, default=12, help='Number of CPU processes to use (default: 12)')
    ...
    for mol in mae_st.molecule:
        ...
        if mol.number in molid_list:
           ...
           #process_all_residues(mol, mae_st, r_dir, mae_fname, in_path, p_dir)
            ...
            res_batches = list(chunk_list(residues, args.nproc))  # Break residues into batches of size n_cpus
            ...
            for batch_num, res_batch in enumerate(res_batches):
                ...
                run_command = ["jaguar", "run"] + job_inputs + ["-jobname", f"batch_{batch_num}", "-HOST", "localhost",'-PARALLEL',str(args.nproc)]
                print(run_command)
                job = launch_job(run_command)#,nproc=args.nproc+1,proj=r_dir)
                job.wait()
                os.chdir('..')
